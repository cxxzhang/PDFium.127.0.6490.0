...
def FindBuildDirWithDepotTools():
  current_dir = os.getcwd()
  if (os.path.exists(os.path.join(current_dir, "build\\depot_tools"))):
    return current_dir
  elif (os.path.exists(os.path.join(current_dir, "..\\build\\depot_tools"))):
    return os.path.join(os.path.join(current_dir, "..\\"))
  return None
...
def GetPrimarySolutionPath():
  """Returns the full path to the primary solution. (gclient_root + pdfium)"""

  gclient_root = FindBuildDirWithDepotTools()# FindGclientRoot(os.getcwd())
  if gclient_root:
    # Some projects' top directory is not named 'pdfium'.
    source_dir_name = 'pdfium' # GetGClientPrimarySolutionName(gclient_root) or 'pdfium'
