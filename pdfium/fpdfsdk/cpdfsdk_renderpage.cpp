...
void CPDFSDK_RenderPage(CPDF_PageRenderContext* pContext,
  ...
  RenderPageImpl(pContext, pPage, matrix, clipping_rect, flags, color_scheme,
                 /*need_to_restore=*/true, /*pause=*/nullptr, false);
}
void CPDFSDK_RenderPageWithContext(CPDF_PageRenderContext* pContext,
	...
	CPDFSDK_RenderPageWithContextEx(pContext, pPage, start_x, start_y, size_x,
		size_y, rotate, flags, color_scheme,
		need_to_restore, pause,
		/*allow_print_widget=*/false);
}



void CPDFSDK_RenderPageWithContextEx(CPDF_PageRenderContext* pContext,
                                   CPDF_Page* pPage,
                                   int start_x,
                                   int start_y,
                                   int size_x,
                                   ...
                                   bool need_to_restore,
                                   CPDFSDK_PauseAdapter* pause,
								   bool allow_print_widget) {
  const FX_RECT rect(start_x, start_y, start_x + size_x, start_y + size_y);
  RenderPageImpl(pContext, pPage, pPage->GetDisplayMatrix(rect, rotate), rect,
                 flags, color_scheme, need_to_restore, pause, allow_print_widget);
}
